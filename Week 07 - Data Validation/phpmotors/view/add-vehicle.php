			...
			<form id="vehicles" method="post" action="/phpmotors/vehicles/index.php">
				...
				<fieldset>
					...
					<div>
						...
						<input type="text" id="make_id" name="invMake" placeholder="E.g.: Jeep" required 
					...
					<div>
						...
						<input type="text" id="model_id" name="invModel" placeholder="E.g.: Wrangler"
						  required <?php if(isset($invModel)){echo "value='$invModel'";}  ?>>
					</div>
					<div>
						<label for="description_id">Description *</label><br />
						<textarea id="description_id" name="invDescription" placeholder="E.g.: JW, is a series of compact and mid-size, four-wheel drive off-road SUVs... etc." rows="6" required
						<?php if(isset($invDescription)){echo "value='$invDescription'";}  ?>></textarea>
					</div>
					<div>
						<label for="image_id">Image Path * </label><br />
						<input type="text" id="image_id" name="invImage" value="/images/no-image.jpg"   required 
						<?php if(isset($invImage)){echo "value='$invImage'";}  ?>>
					</div>
					<div>
						<label for="thumbnail_id">Thumbnail Path * </label><br />
						<input type="text" id="thumbnail_id" name="invThumbnail" value="/images/no-image-tn.jpg" required <?php if(isset($invThumbnail)){echo "value='$invThumbnail'";}  ?> >
					</div>
					<div>
						<label for="price_id">Price *</label><br />
						<input type="text" id="price_id" name="invPrice" placeholder="E.g.: 28475.45"  pattern="([0-9]+(?:\.[0-9]*)?)" required
						<?php if(isset($invPrice)){echo "value='$invPrice'";}  ?>>
					</div>
					<div>
						<label for="stock_id"># In Stock * </label><br />
						<input type="text" id="stock_id" name="invStock" placeholder="E.g. in units: 4"  pattern="(^\d{1,10}$)" required  
						<?php if(isset($invStock)){echo "value='$invStock'";}  ?>>
					</div>
					<div>
						<label for="color_id">Color * </label><br />
						<input type="text" id="color_id" name="invColor" placeholder="E.g.: Black" required 
