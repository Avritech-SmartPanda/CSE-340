...
function buildVehicleByIdDisplay($vehicle)
{
  foreach ($vehicle as $data) {
    ...
    $dv .= "<img src='$data[imgPath]' width='600' alt='Image of $data[invMake] $data[invModel]'>";
...
function buildReviewDisplay($itemReviews)
{
  ...
  foreach ($itemReviews as $review) {
    ...
    $rd .= "<li class='review-list-item my-30'><span class='meta-text'><strong>$screenName</strong> wrote on $date:<br></span>";
    $rd .= "<div class='review-list-item-text'>$reviewText</div></li>";
  }
  $rd .= "</ul>";
  return $rd;
}
...
function adminReviewDisplay($reviews)
{
  $rd = '<ul id="product-reviews-table">';
  foreach ($reviews as $review) {
    ...
    $date = date("F jS, Y", strtotime($review['reviewDate']));
    $reviewText = $review['reviewText'];
    $rd .= "<li class='review-list-item-date'><span class='meta-text'>(Reviewed on $date)</br></span>";
    $rd .= "<span class='review-list-item-text'>$reviewText</span>";
    $rd .= "<span class='review-actions'><a href=\"/phpmotors/reviews/index.php/?action=edit-review&id=$reviewId\">Edit</a> |
        <a href=\"/phpmotors/reviews/index.php/?action=delete-review&id=$reviewId\"> Delete</a>  </span></li>";
  }
  $rd .= "</ul>";
  return $rd;
}
...
function buildThumbnailsDisplay($vehicleThumbnails)
{
  $dv = '<ul class="thumbnails-display">';
  foreach ($vehicleThumbnails as $thumbnail) {
    $dv .= '<li class="thumbnail-content">';
    $dv .= "<img src='../images/vehicles/$thumbnail[imgName]' alt='Image of $thumbnail[invMake] $thumbnail[invModel] on phpmotors.com'>";
    $dv .= '</li>';
  }
  $dv .= '</ul>';
  return $dv;
