...
/** Get Thumbnails Information from images table. */
function getThumbnails($invId) {
	$db = phpmotorsConnect();
	$sql = 'SELECT imgId, images.invId, imgName, imgPath, imgDate, invMake, invModel
	FROM images
	JOIN inventory
	ON inventory.invId = images.invId
	WHERE images.invId = :invId
	AND imgName LIKE "%-tn%"';
	$stmt = $db->prepare($sql);
	$stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
	$stmt->execute();
	$thumbnailsArray = $stmt->fetchAll(PDO::FETCH_ASSOC);
	$stmt->closeCursor();
