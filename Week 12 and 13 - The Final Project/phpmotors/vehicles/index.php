 <?php
  ...
  switch ($action) {
    ...
    case 'classification':
      ...
      if (!count($vehicles)) {
        $message = "<p class='server-message'>Sorry, no $classificationName could be found. </p>";
      } else {
    ...
    case 'details':
      ...
      $vehicleReviews = getInventoryItemReviews($invId);

      foreach ($vehicle as $data) {
        $invMake = $data['invMake'];
        $invModel = $data['invModel'];
      }
      if (!count($vehicle)) {
        $message = "<p class='error'>Sorry, no such vehicle type could be found.</p>";
      } else {
        $vehicleDisplay = buildVehicleByIdDisplay($vehicle);
        $thumbnailsDisplay = buildThumbnailsDisplay($vehicleThumbnails);
        $reviewsDisplay = buildReviewDisplay($vehicleReviews);
      }
